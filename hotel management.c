...
int main()
{
    ...
    system("cls");
    time_t timer = time(NULL);/*to get the present local time*/     begin();
    return 0;
}
void begin(void)
{
...
void book_room(void)
{
    ...
    if(person.roomno == 0)
    {
        ...
        if(type_of_rooms==4)         begin();
    }
    else         printf("you have already booked a room");
}
void program(void)
{
    system("cls");
...
void GuessGame(int amount_bet, int* inhand_cash)
{
  ...
  the random number for random index*/    for (i = 0; i < 5; i++)
    {
        x = rand() % 3;
        y = rand() % 3;
        temp = num[x];
...
void mini_casino()
{
    int amount_bet, inhand_cash;  /*   You have to guess the right number among 3 numbers.   The position where right number is is   named as 'R' and rest two are named as 'N'   If your guess is wrong, you loose the   amount_bet from your inhand_cash   If you guess it right, you win   twice the amount_bet in your inhand_cash   Keep playing and keep winning   until you go out of cash  */      printf("\n////////-WELCOME TO MINI CASINO-\\\\\\\\\\\\ \n");
    ...
    while (inhand_cash > 0)
    {
        ...
        else     GuessGame(amount_bet, &inhand_cash);
    }
